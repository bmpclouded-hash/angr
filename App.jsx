...
import {
  ...
  Navigation, Crosshair, Minus, Medal, TrendingUp, Mail, KeyRound, AlertCircle
} from "lucide-react";
...
/* ---------------------------------------------------------------
   ...
   Backend: Supabase (real auth + Postgres, row-level security scoped
   to each signed-in user). See angr-schema.sql for the table setup.
------------------------------------------------------------------*/

const SUPABASE_URL = "https://zhyfkhpcktfxtkmeiaul.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_LIdiFsJNg9wwBM3WB57E7w_B0lhaeU1";
...
const rowToSpot = (r) => ({ id: r.id, name: r.name, x: Number(r.x), y: Number(r.y), species: r.species || [], bait: r.bait || "—", bestTime: r.best_time || "—", notes: r.notes || "", privacy: r.privacy || "private", trips: r.trips || 0 });
const spotToRow = (s, uid_) => ({ id: s.id, user_id: uid_, name: s.name, x: s.x, y: s.y, species: s.species, bait: s.bait, best_time: s.bestTime, notes: s.notes, privacy: s.privacy, trips: s.trips });

const rowToTrip = (r) => ({ id: r.id, name: r.name, location: r.location, spotId: r.spot_id, species: r.species, startTime: new Date(r.start_time).getTime(), endTime: r.end_time ? new Date(r.end_time).getTime() : null, weather: r.weather, notes: r.notes || "" });
const tripToRow = (t, uid_) => ({ id: t.id, user_id: uid_, name: t.name, location: t.location, spot_id: t.spotId || null, species: t.species || null, start_time: new Date(t.startTime).toISOString(), end_time: t.endTime ? new Date(t.endTime).toISOString() : null, weather: t.weather, notes: t.notes });

const rowToCatch = (r) => ({ id: r.id, tripId: r.trip_id, species: r.species, length: Number(r.length), weight: Number(r.weight), bait: r.bait, location: r.location, time: new Date(r.time).getTime(), photos: r.photos || [], waterCondition: r.water_condition, weatherNote: r.weather_note, notes: r.notes || "", gear: r.gear || {} });
const catchToRow = (c, uid_) => ({ id: c.id, user_id: uid_, trip_id: c.tripId || null, species: c.species, length: c.length, weight: c.weight, bait: c.bait, location: c.location, time: new Date(c.time).toISOString(), photos: c.photos || [], water_condition: c.waterCondition, weather_note: c.weatherNote, notes: c.notes, gear: c.gear || {} });

const rowToGear = (r) => ({ id: r.id, category: r.category, brand: r.brand, model: r.model, name: r.name, length: r.length, power: r.power, action: r.action, ratio: r.ratio, color: r.color, type: r.type, size: r.size });
const gearToRow = (g, uid_) => ({ id: g.id, user_id: uid_, category: g.category, brand: g.brand || null, model: g.model || null, name: g.name || null, length: g.length || null, power: g.power || null, action: g.action || null, ratio: g.ratio || null, color: g.color || null, type: g.type || null, size: g.size || null });

const rowToSpecies = (r) => ({ id: r.id, name: r.name, water: r.water, avgSize: r.avg_size, habitat: r.habitat, seasons: r.seasons, bait: r.bait, technique: r.technique, fact: r.fact, photo: r.photo, custom: true });
const speciesToRow = (s, uid_) => ({ id: s.id, user_id: uid_, name: s.name, water: s.water, avg_size: s.avgSize || null, habitat: s.habitat || null, seasons: s.seasons || null, bait: s.bait || null, technique: s.technique || null, fact: s.fact || null, photo: s.photo || null });

async function fetchAllForUser(userId) {
  const [profileRes, spotsRes, tripsRes, catchesRes, gearRes, speciesRes, achRes] = await Promise.all([
    supabase.from("profiles").select("*").eq("id", userId).maybeSingle(),
    supabase.from("spots").select("*").order("created_at"),
    supabase.from("trips").select("*").order("start_time"),
    supabase.from("catches").select("*").order("time"),
    supabase.from("gear").select("*").order("created_at"),
    supabase.from("custom_species").select("*").order("created_at"),
    supabase.from("achievements").select("achievement_id"),
  ]);
  return {
    user: profileRes.data ? { name: profileRes.data.name, units: profileRes.data.units, notifications: profileRes.data.notifications } : { name: "Angler", units: "imperial", notifications: true },
    spots: (spotsRes.data || []).map(rowToSpot),
    trips: (tripsRes.data || []).map(rowToTrip),
    catches: (catchesRes.data || []).map(rowToCatch),
    gear: (gearRes.data || []).map(rowToGear),
    customSpecies: (speciesRes.data || []).map(rowToSpecies),
...
function AuthScreen() {
  ...
  async function submit() {
    setError(""); setNotice("");
    if (!email.trim() || (mode !== "reset" && !password)) { setError("Fill in your email and password."); return; }
    setBusy(true);
    try {
      if (mode === "signup") {
        if (!name.trim()) { setError("What should we call you?"); setBusy(false); return; }
  ...
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 28, textAlign: "center" }}>
      <svg width="64" height="64" viewBox="0 0 40 40" fill="none" style={{ marginBottom: 16 }}>
        <path d="M8 22c0-7 5-14 12-14 5 0 8 3 8 3" stroke="#C17A46" strokeWidth="2.5" strokeLinecap="round" />
        <path d="M28 11c1.5 0 3 1 3 3s-2 3.5-2 5c0 2 1.5 3 1.5 3" stroke="#C17A46" strokeWidth="2.5" strokeLinecap="round" />
      ...
      <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 30, letterSpacing: 1.5, color: "#F2EFE6" }}>ANGR</div>
      <div style={{ color: "#9CA394", fontSize: 13, marginTop: 6, marginBottom: 22, maxWidth: 280 }}>Your fishing log, spots, and gear — synced to your account.</div>

      {mode !== "reset" && (
        <div style={{ display: "flex", gap: 8, width: "100%", maxWidth: 300, marginBottom: 16 }}>
          {["login", "signup"].map((m) => (
            <button key={m} onClick={() => { setMode(m); setError(""); setNotice(""); }} style={{ flex: 1, padding: "9px 0", borderRadius: 10, border: "1px solid #2C3226", background: mode === m ? "#26402C" : "#161A12", color: mode === m ? "#BFE0C6" : "#9CA394", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
      ...
      <div style={{ width: "100%", maxWidth: 300, textAlign: "left" }}>
        {mode === "signup" && <TextField label="Name" value={name} onChange={setName} placeholder="What should we call you?" icon={UserIcon} />}
        <TextField label="Email" value={email} onChange={setEmail} placeholder="you@email.com" type="email" icon={Mail} />
        {mode !== "reset" && <TextField label="Password" value={password} onChange={setPassword} placeholder="••••••••" type="password" icon={KeyRound} />}

        {error && <div style={{ display: "flex", gap: 6, alignItems: "flex-start", color: "#E3A576", fontSize: 12, marginBottom: 12 }}><AlertCircle size={14} style={{ flexShrink: 0, marginTop: 1 }} /> {error}</div>}
        {notice && <div style={{ display: "flex", gap: 6, alignItems: "flex-start", color: "#8FC1E3", fontSize: 12, marginBottom: 12 }}><Info size={14} style={{ flexShrink: 0, marginTop: 1 }} /> {notice}</div>}

        <BigButton icon={ChevronRight} label={busy ? "Please wait…" : mode === "signup" ? "Create Account" : mode === "reset" ? "Send Reset Email" : "Log In"} onClick={busy ? undefined : submit} />
      </div>

      {mode === "login" && <button onClick={() => { setMode("reset"); setError(""); setNotice(""); }} style={{ background: "none", border: "none", color: "#8FAE7E", fontSize: 12, marginTop: 14, cursor: "pointer" }}>Forgot password?</button>}
      {mode === "reset" && <button onClick={() => { setMode("login"); setError(""); setNotice(""); }} style={{ background: "none", border: "none", color: "#8FAE7E", fontSize: 12, marginTop: 14, cursor: "pointer" }}>Back to log in</button>}
    </div>
  );
}
...
function SyncErrorBanner({ message, onClose }) {
  useEffect(() => { const t = setTimeout(onClose, 4000); return () => clearTimeout(t); }, []);
  return (
    <div style={{ position: "fixed", top: 16, left: 16, right: 16, zIndex: 60, display: "flex", justifyContent: "center" }}>
      <div style={{ background: "#2E1B14", border: "1px solid #B5654A", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, maxWidth: 380, boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
        <AlertCircle size={16} color="#E3A576" style={{ flexShrink: 0 }} />
        <div style={{ fontSize: 12.5, color: "#E3A576" }}>{message}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  async function hydrate(uid_) {
    setLoading(true);
    try {
      const fetched = await fetchAllForUser(uid_);
      setData(fetched);
    } catch (e) {
      setSyncError("Couldn't load your data — check your connection and reload.");
    }
    setLoading(false);
  }
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session: s } }) => {
      setSession(s);
      if (s) hydrate(s.user.id); else setLoading(false);
    });
    const { data: sub } = supabase.auth.onAuthStateChange((_event, s) => {
      setSession(s);
      if (s) hydrate(s.user.id);
      else { setData(emptyData()); setActiveTrip(null); setLoading(false); }
    });
  ...
  const checkAchievements = useCallback((nextData) => {
    ...
    if (newly.length > 0) {
      ...
      setToast(newly[0]);
      if (userId) {
        supabase.from("achievements").insert(newly.map((a) => ({ user_id: userId, achievement_id: a.id }))).then(({ error }) => {
          if (error) setSyncError("Achievement unlocked, but couldn't sync it — it'll retry next time you're active.");
        });
      }
      return updated;
    }
  ...
  function handleStartTrip(trip) {
    ...
    setTab("home");
    supabase.from("trips").insert(tripToRow(trip, userId)).then(({ error }) => { if (error) setSyncError("Trip started, but couldn't sync — check your connection."); });
  }
  ...
  function handleEndTrip() {
    ...
    setData((d) => {
      const trips = d.trips.map((t) => t.id === activeTrip.id ? { ...t, endTime } : t);
      let spots = d.spots;
      if (activeTrip.spotId) spots = spots.map((s) => s.id === activeTrip.spotId ? { ...s, trips: s.trips + 1 } : s);
      const next = { ...d, trips, spots };
      return checkAchievements(next);
    });
    supabase.from("trips").update({ end_time: new Date(endTime).toISOString() }).eq("id", activeTrip.id).then(({ error }) => { if (error) setSyncError("Couldn't sync the trip end — it'll retry next time you're active."); });
    if (activeTrip.spotId) {
      const spot = data.spots.find((s) => s.id === activeTrip.spotId);
      if (spot) supabase.from("spots").update({ trips: spot.trips + 1 }).eq("id", spot.id).then(() => {});
    }
    setActiveTrip(null);
  }
  ...
  function handleLogCatch(c) {
    ...
    supabase.from("catches").insert(catchToRow(withTrip, userId)).then(({ error }) => { if (error) setSyncError("Catch saved locally, but couldn't sync — check your connection."); });
  }

  function handleAddSpot(spot) {
    setData((d) => checkAchievements({ ...d, spots: [...d.spots, spot] }));
    supabase.from("spots").insert(spotToRow(spot, userId)).then(({ error }) => { if (error) setSyncError("Spot saved locally, but couldn't sync — check your connection."); });
  }
  function handleDeleteSpot(id) {
    setData((d) => ({ ...d, spots: d.spots.filter((s) => s.id !== id) }));
    supabase.from("spots").delete().eq("id", id).then(({ error }) => { if (error) setSyncError("Couldn't sync that deletion — it may reappear on next sync."); });
  }
  function handleDeleteCatch(c) {
    ...
    supabase.from("catches").delete().eq("id", c.id).then(({ error }) => { if (error) setSyncError("Couldn't sync that deletion — it may reappear on next sync."); });
  }
  function handleEditCatchSave(updated) {
    setData((d) => checkAchievements({ ...d, catches: d.catches.map((c) => (c.id === updated.id ? updated : c)) }));
    setEditingCatch(null);
    supabase.from("catches").update(catchToRow(updated, userId)).eq("id", updated.id).then(({ error }) => { if (error) setSyncError("Couldn't sync those changes — check your connection."); });
  }
  function handleDeleteTrip(t) {
    ...
    supabase.from("catches").delete().eq("trip_id", t.id).then(() => {
      supabase.from("trips").delete().eq("id", t.id).then(({ error }) => { if (error) setSyncError("Couldn't sync that deletion — it may reappear on next sync."); });
    });
    if (t.endTime && t.spotId) {
      const spot = data.spots.find((s) => s.id === t.spotId);
      if (spot) supabase.from("spots").update({ trips: Math.max(0, spot.trips - 1) }).eq("id", spot.id).then(() => {});
    }
  }
  function handleEditTripSave(updated) {
    setData((d) => ({ ...d, trips: d.trips.map((t) => (t.id === updated.id ? updated : t)) }));
    setEditingTrip(null);
    supabase.from("trips").update(tripToRow(updated, userId)).eq("id", updated.id).then(({ error }) => { if (error) setSyncError("Couldn't sync those changes — check your connection."); });
  }
  function handleAddSpecies(sp) {
    setData((d) => ({ ...d, customSpecies: [...d.customSpecies, sp] }));
    supabase.from("custom_species").insert(speciesToRow(sp, userId)).then(({ error }) => { if (error) setSyncError("Species saved locally, but couldn't sync — check your connection."); });
  }
  function handleUpdateSpecies(sp) {
    setData((d) => ({ ...d, customSpecies: d.customSpecies.map((s) => s.id === sp.id ? sp : s) }));
    supabase.from("custom_species").update(speciesToRow(sp, userId)).eq("id", sp.id).then(({ error }) => { if (error) setSyncError("Couldn't sync those changes — check your connection."); });
  }
  function handleDeleteSpecies(id) {
    setData((d) => ({ ...d, customSpecies: d.customSpecies.filter((s) => s.id !== id) }));
    supabase.from("custom_species").delete().eq("id", id).then(({ error }) => { if (error) setSyncError("Couldn't sync that deletion — it may reappear on next sync."); });
  }
  function handleAddGear(g) {
    setData((d) => ({ ...d, gear: [...d.gear, g] }));
    supabase.from("gear").insert(gearToRow(g, userId)).then(({ error }) => { if (error) setSyncError("Gear saved locally, but couldn't sync — check your connection."); });
  }
  function handleDeleteGear(id) {
    setData((d) => ({ ...d, gear: d.gear.filter((g) => g.id !== id) }));
    supabase.from("gear").delete().eq("id", id).then(({ error }) => { if (error) setSyncError("Couldn't sync that deletion — it may reappear on next sync."); });
  }
  function handleUpdateUser(u) {
    setData((d) => ({ ...d, user: u }));
    if (userId) supabase.from("profiles").update({ name: u.name, units: u.units, notifications: u.notifications }).eq("id", userId).then(({ error }) => { if (error) setSyncError("Couldn't sync your settings — check your connection."); });
  }
  ...
  if (!session) {
    return <div style={shell}>{fontImports}<AuthScreen /></div>;
  }
  ...
  return (
    <div style={shell}>
      ...
      {syncError && <SyncErrorBanner message={syncError} onClose={() => setSyncError(null)} />}
    </div>
  );
}
